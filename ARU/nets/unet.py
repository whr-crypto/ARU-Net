...
class SEConv(nn.Module):
    def __init__(self, inch, outch):
        ...
        self.attention = senet(channel=inch)
...
class self_attention_plus(nn.Module):
    def __init__(self, channel):
        super(self_attention_plus, self).__init__()
        self.channel = channel
        self.pointwiseConv = nn.Sequential(
            nn.Conv2d(self.channel, self.channel, kernel_size=1, padding=0, bias=True),
            nn.BatchNorm2d(self.channel),
    ...
    def forward(self, x):
        ...
        x_2 = torch.cat((x1, x3), dim=1)
        x12 = self.relu(x_1 * x_2)
        x12 = self.voteConv(x12)
        x_3 = self.relu(torch.cat((x2, x3), dim=1))
        x_3 = self.voteConv(x_3)
        x = x + x_3 * x12
